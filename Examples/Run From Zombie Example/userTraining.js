...
function simulateEnvStep(action, currentStep) {
  ...
  function userTrainMain(epNum,stepSize,batchSize) {
    ...
    if (epNum && !isNaN(epNum) && epNum > 0) {userTrainingControl.episodes = epNum}
    if (stepSize && !isNaN(stepSize) && stepSize > 0) {userTrainingControl.maxStepCount = stepSize}
    if (batchSize && !isNaN(batchSize) && batchSize > 0) {agent.batch_size = batchSize}

    observationSpace.initUpdate()
    userTrainingControl.currentStep = 0;
    userTrainingControl.currentEpisode = 0;
    userTrainingControl.newEpisode = true;
    userTrainingControl.animationTime = setInterval(animate, 30);
