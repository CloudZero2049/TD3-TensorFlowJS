...
function simulateEnvStep(action, currentStep) {
    ...
    const x = parseFloat(player.speed * action[0].toFixed(8)); 
    const y = parseFloat(player.speed * action[1].toFixed(8));
    ...
    if(player.sensor){
      ...
      for (let i = 0; i < offsets.length; i++){
        osNS[0][i+6] = parseFloat((offsets[i]).toFixed(8));
      }
    }
    ...
    */
    ...
    if (penalty > 10) {penalty = 10} // failsafe
    if (reward > 10) {reward = 10} // failsafe
